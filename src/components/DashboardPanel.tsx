...
export function DashboardPanel({ contacts }: DashboardPanelProps) {
  const stats = useMemo(() => {
    ...
    let totalConfidence = 0, validPhones = 0, withEmail = 0;
    ...
    return {
      total: contacts.length, unique: clean.length, dupes: contacts.length - clean.length,
      avgConfidence: clean.length > 0 ? Math.round(totalConfidence / clean.length) : 0,
      validPhones, withEmail, countries: sortedCountries, sources: sortedSources,
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        ...
        <SummaryCard label="Únicos" value={stats.unique} color="text-success" />
        <SummaryCard label="Duplicados" value={stats.dupes} color="text-warning" />
          ...
          <CardContent className="space-y-2">
            {stats.countries.map(([country, count]) => (
              <div key={country} className="flex items-center gap-2">
                <span className="text-xs w-8 font-mono text-foreground">{country}</span>
                <div className="flex-1 h-5 bg-muted rounded-sm overflow-hidden">
                  <div className="h-full bg-primary/60 rounded-sm transition-all" style={{ width: `${(count / stats.maxCountryCount) * 100}%` }} />
                </div>
                <span className="text-xs text-muted-foreground w-10 text-right">{count}</span>
              </div>
            ))}
            {stats.countries.length === 0 && <p className="text-xs text-muted-foreground">Sin datos de país</p>}
          </CardContent>
        </Card>
...
function SummaryCard({ label, value, color }: { label: string; value: number | string; color?: string }) {
  return (
    <div className="rounded-lg border bg-card p-4 text-center">
      <p className={`text-2xl font-bold ${color || "text-foreground"}`}>{typeof value === "number" ? value.toLocaleString() : value}</p>
